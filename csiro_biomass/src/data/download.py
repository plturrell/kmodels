"""Download helpers for the CSIRO Image2Biomass Kaggle competition.

The script wraps the Kaggle API so the dataset can be fetched and extracted
into a local workspace directory. Usage example:

    python -m competitions.csiro_biomass.src.data.download \\
        --download-dir data/csiro_biomass/raw --extract

Make sure the environment has `kaggle` installed and that your API token is
available (typically `~/.kaggle/kaggle.json`).
"""
...
def _authenticate():
    ...
    try:
        api.authenticate()
    except Exception as exc:  # pragma: no cover - credentials failure path
        msg = (
            "Failed to authenticate with Kaggle. Confirm that your API token is "
            "available at ~/.kaggle/kaggle.json and that it has correct "
            "permissions."
        )
        raise RuntimeError(msg) from exc
    return api
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Competition slug (default: csiro-biomass).",
    )
    parser.add_argument(
        "--download-dir",
        type=Path,
        default=Path("competitions/csiro_biomass/data/raw"),
        help="Directory to save downloads (default: data/csiro_biomass/raw).",
    )
    parser.add_argument(
        "--file",
        dest="files",
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    except Exception as exc:  # pragma: no cover - high level CLI protection
        LOGGER.error("%s", exc)
        return 1
    return 0
