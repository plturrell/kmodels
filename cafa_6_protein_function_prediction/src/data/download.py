"""Download helpers for the CAFA 6 Protein Function Prediction competition.

Usage example:

    python -m competitions.cafa_6_protein_function_prediction.src.data.download \
        --extract

Kaggle credentials must be available at ``~/.kaggle/kaggle.json`` and the Kaggle
CLI package installed before running this module.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Download data for the CAFA 6 Protein Function Prediction competition."
    )
    parser.add_argument(
        "--competition",
