"""Helpers to load NFL Big Data Bowl 2026 datasets into pandas."""
...
DEFAULT_DATA_ROOT = Path(
    "competitions/nfl_big_data_bowl_2026_prediction/data/raw/nfl-big-data-bowl-2026-prediction"
)
...
def _resolve_root(base_dir: Optional[Path | str]) -> Path:
    ...
    if not root.exists():
        raise FileNotFoundError(
            f"Data directory {root} not found. Run the download script first."
        )
    return root
...
def available_train_weeks(base_dir: Optional[Path | str] = None) -> List[Tuple[int, int]]:
    ...
    train_dir = _resolve_root(base_dir) / "train"
    if not train_dir.exists():
        raise FileNotFoundError(
            f"Train directory {train_dir} not found. Check your data download."
        )
    weeks = set()
...
def _week_filepath(
    ...
    path = _resolve_root(base_dir) / "train" / f"{prefix}_{week_tag}.csv"
    if not path.exists():
...
def load_test_input(
    ...
    """Load the test input trajectories (without targets)."""
    path = _resolve_root(base_dir) / "test_input.csv"
    if not path.exists():
        raise FileNotFoundError(f"Could not locate file {path}")
    return pd.read_csv(path, **read_csv_kwargs)
...
def load_submission_format(
    ...
    """Load the submission index (``test.csv``)."""
    path = _resolve_root(base_dir) / "test.csv"
    if not path.exists():
        raise FileNotFoundError(f"Could not locate file {path}")
    return pd.read_csv(path, **read_csv_kwargs)
