...
class InteractionGNNLayer(nn.Module):
    ...
    def forward(self, node_feats: torch.Tensor, positions: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        outputs = torch.zeros_like(node_feats)
        for b in range(batch):
            valid_idx = mask[b].nonzero(as_tuple=False).squeeze(-1)
            if valid_idx.numel() == 0:
                outputs[b] = node_feats[b]
                continue

            nf = node_feats[b, valid_idx]
            pos = positions[b, valid_idx]

            q = self.query(nf).view(valid_idx.numel(), self.heads, -1).transpose(0, 1)  # [H, V, D]
            k = self.key(nf).view(valid_idx.numel(), self.heads, -1).transpose(0, 1)
            v = self.value(nf).view(valid_idx.numel(), self.heads, -1).transpose(0, 1)

            deltas = _pairwise_deltas(pos.unsqueeze(0)).squeeze(0)
            distances = _pairwise_distances(deltas).unsqueeze(-1)
            direction = F.normalize(deltas, dim=-1, eps=1e-6)
            edge_geom = torch.cat([direction, distances], dim=-1)
            edge_bias = self.edge_mlp(edge_geom).mean(-1)  # [V, V]
            edge_bias = edge_bias.unsqueeze(0).expand(self.heads, -1, -1)

            attn_scores = torch.einsum('hvd,hud->hvu', q, k) / self.scale
            attn_scores = attn_scores + edge_bias
            attn_scores = attn_scores - attn_scores.max(dim=-1, keepdim=True).values
            attn_weights = torch.softmax(attn_scores, dim=-1)
            attn_weights = self.dropout(attn_weights)

            agg = torch.einsum('hvu,hud->hvd', attn_weights, v)
            agg = agg.transpose(0, 1).contiguous().view(valid_idx.numel(), -1)
            agg = self.out_proj(agg)

            outputs[b, valid_idx] = self.norm(nf + agg)
