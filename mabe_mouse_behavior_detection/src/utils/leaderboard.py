"""Fetch and summarise Kaggle leaderboard standings for MABe mouse behavior detection."""
...
def _parse_leaderboard_csv(data: Iterable[str]) -> List[LeaderboardEntry]:
    ...
    for _, row in df.iterrows():
        members = [
            name.strip()
            for name in str(row["TeamMemberUserNames"]).split(",")
            if name
        ]
        entries.append(
            LeaderboardEntry(
...
def fetch_leaderboard(competition: str = DEFAULT_COMPETITION) -> List[LeaderboardEntry]:
    """Download and parse the current public leaderboard."""
    api = _authenticate()
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            with zf.open(csv_name) as fh:
                ...
                data = text.readlines()
    return _parse_leaderboard_csv(data)
...
def summarise(
    ...
    for entry in entries[:top_n]:
        lines.append(
            f"{entry.rank:>4}  {entry.team_name:<30.30}  {entry.score:>8.3f}  {entry.submission_count:>3}"
        )
    ...
    if user:
        ...
        if found:
            lines.append(
                f"Your team '{found.team_name}' is currently #{found.rank} with score {found.score:.3f} "
                f"(last submit {found.last_submission})."
            )
        else:
