"""CLI helpers to pull the MABe mouse behavior detection dataset via the Kaggle API."""
...
def _authenticate():
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to authenticate with Kaggle. Confirm ~/.kaggle/kaggle.json exists, "
            "has permissions 0600, and you have joined the competition."
        ) from exc
    return api
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Download data for the MABe mouse behavior detection competition."
    )
    parser.add_argument(
        "--competition",
    ...
    parser.add_argument(
        ...
        help=f"Directory to save downloads (default: {raw_data_dir().as_posix()}).",
    )
    parser.add_argument(
        "--file",
        dest="files",
